...
CREATE TABLE users (
    ...
    business_name VARCHAR(100),              -- اسم العمل التجاري (اختياري)
    password_hash VARCHAR(255) NOT NULL,
    status VARCHAR(20) DEFAULT 'active',     -- active, inactive, suspended
    last_login TIMESTAMP WITH TIME ZONE,
    profile_image VARCHAR(255),
    settings JSONB DEFAULT '{}',             -- إعدادات المستخدم العامة
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP
);
-- جدول رموز إعادة تعيين كلمة المرور (منفصل)
...
CREATE TABLE partner_vehicle_requests (
    ...
    purpose TEXT NOT NULL,                       -- الغرض من الاستئجار
    ...
    approved_by INTEGER REFERENCES users(id),    -- مدير الأسطول الذي وافق
...
CREATE TABLE drivers (
    id SERIAL PRIMARY KEY,
    user_id INTEGER REFERENCES users(id),    -- للسائقين التابعين لشركة
...
CREATE TABLE driver_job_applications (
    id SERIAL PRIMARY KEY,
    driver_id INTEGER REFERENCES drivers(id) ON DELETE CASCADE,
    fleet_manager_id INTEGER REFERENCES users(id) ON DELETE CASCADE,
...
CREATE TABLE driver_reviews (
    id SERIAL PRIMARY KEY,
    driver_id INTEGER REFERENCES drivers(id),
    reviewer_id INTEGER REFERENCES users(id),  -- مدير الأسطول
...
CREATE TABLE vehicle_driver_logs (
    ...
    verified_by INTEGER REFERENCES users(id),   -- في حالة override
    notes TEXT,
    created_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP WITH TIME ZONE DEFAULT CURRENT_TIMESTAMP,
...
CREATE TABLE trips (
    ...
    user_id INTEGER REFERENCES users(id),       -- منشئ الرحلة
    vehicle_id INTEGER REFERENCES vehicles(id),
...
CREATE TABLE trip_device_logs (
    ...
    position_id INTEGER REFERENCES positions(id),  -- من Traccar
...
CREATE TABLE maintenance_schedules (
    ...
    assigned_to INTEGER REFERENCES users(id),   -- المسؤول عن الصيانة
...
CREATE TABLE partner_trip_requests (
    ...
    assigned_fleet_id INTEGER REFERENCES fleets(id),
    assigned_vehicle_id INTEGER REFERENCES vehicles(id),
